...
export class vierGewinntRoom extends Room {
...
/*    if (client.sessionId === this.player1.id) {
    ...
    if (client.sessionId === this.player2.id3) {
      if (!rules[data.message]) {
        return this.broadcast(`${client.sessionId} sagt: ${data.message}`);
      }
       else {
    ...
    // if (client.sessionId) { //TODO istDran
    //   if (!rules[data.message]) {
    ...
    console.log(client.sessionId + "legt" + data.message);
    if (client.sessionId == this.player1.id) this.send(this.player2.client, "koord: " + data.message);
